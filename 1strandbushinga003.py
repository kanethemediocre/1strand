...
while (CurrentLayer*LayerHeight < height):
    ...
    j=1
    i=0
    radius = InnerDiameter/2 + (j-0.5)*ActualExtrusionWidth #j=1 on first go, so start is radius+half extrusion width
    xpath.append(centerx+radius)
    ypath.append(centery)
    zpath.append(CurrentHeight-LayerHeight*0.75) #1st point at .75 height for blend
    while (j<=perimeters):
        i=0
        radius = InnerDiameter/2 + (j-0.5)*ActualExtrusionWidth
        while (i<(N-1)): #kludge
    ...
    j=1
    i=0
    radius = OuterDiameter/2 - (j-0.5)*ActualExtrusionWidth #j=1 on first go, so start is radius-half extrusion width
    xpath.append(centerx+radius)
    ypath.append(centery)
